...
export default function CrossSellAnalyticsPage() {
  const [presetDays, setPresetDays] = useState(30);
  const [branchId, setBranchId] = useState<string>("all");

  const branches = useQuery(api.analytics.crossSellAnalytics.listBranchesForFilter);
  ...
  const stats = useQuery(api.analytics.crossSellAnalytics.getCrossSellAnalytics, {
    branchId: branchId !== "all" ? (branchId as Id<"branches">) : undefined,
    startDate,
    endDate,
  });
